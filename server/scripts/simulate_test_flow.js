...
function req(method, path, data, headers){
  return new Promise((resolve, reject)=>{
    ...
    const options = { hostname: HOST, port: PORT, path: path, method: method, headers: Object.assign({ 'Accept': 'application/json' }, headers || {}) };
    ...
    const r = http.request(options, res => {
      ...
      res.on('end', () => {
        ...
        if(ct.includes('application/json')){
          try{ return resolve({ status: res.statusCode, body: JSON.parse(body), headers: res.headers }); }catch(e){ return resolve({ status: res.statusCode, body: body, headers: res.headers }); }
        }
        return resolve({ status: res.statusCode, body: body, headers: res.headers });
      });
    });
...
(async function main(){
  try{
    ...
    const studentCookie = extractCookie(studentRes.headers);
    ...
    const sessRes = await req('POST','/api/exam-sessions', { student_id: student.id, test_id: test.id }, { Cookie: studentCookie });
    ...
    for(const q of questions){
      ...
      const sub = await req('POST','/api/submit-answer', payload, { Cookie: studentCookie });
      console.log(` -> question ${q.id} submit status=${sub.status} resp=${JSON.stringify(sub.body)}`);
    }

    console.log('7) Finishing session...');
    const fin = await req('PUT', `/api/exam-sessions/${encodeURIComponent(session.id)}/finish`, null, { Cookie: studentCookie });
    console.log(' -> finish status', fin.status, 'body', fin.body);

    console.log('8) Checking student summary...');
    const summary = await req('GET', `/api/tests/${encodeURIComponent(test.id)}/summary?student_id=${encodeURIComponent(student.id)}&session_id=${encodeURIComponent(session.id)}`, null, { Cookie: studentCookie });
    console.log(' -> summary response status', summary.status);
